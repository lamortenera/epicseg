...
getColors <- function(counts, clusts, nstates, markToCol=list(red="H3K4me3", green4="H3K36me3", blue=c("H3K9me3", "H3K27me3"), gold="H3K4me1"), background="gray"){
	if (length(clusts) != ncol(counts)) stop("counts and clusts don't match")
	if (any(clusts <= 0 | clusts > nstates)) stop("invalid clusts label")
	if (is.null(rownames(counts))) stop("counts matrix must have rownames set")
	available_marks <- rownames(counts)
	for (marks in markToCol){
		if (!any(marks %in% available_marks)) stop("cannot find matching marks")
	}
	
	avgCounts <- avgCountsPerClust(counts, clusts)
	rownames(avgCounts) <- rownames(counts)
	#clusters that never occur should have NaNs in the matrix avgCounts
	validClust <- apply(avgCounts, 2, function(v) all(is.finite(v)))
	avgCounts <- avgCounts[,validClust, drop=F]
	n <- ncol(avgCounts)
	...
	for (i in seq_along(markToCol)){
		marks <- markToCol[[i]]
		scores <- colSums(avgCounts[intersect(marks, available_marks), , drop=F])
	...
	if (!is.null(background)){
		#the indices in the array scores correspond to the indices for the columns in avgCounts
	...
	#dmat <- kfoots:::KL_dist_mat(avgCounts+1e-6, 1)
	dmat <- as.matrix(dist(t(log(avgCounts+1e-6))))
	#similarity matrix
